...
| [`portals-pages`](./portals-pages/SKILL.md) | Creating and editing content pages on a data-fair **portals v2** portal from the manager (`/portals-manager`): page-config API, admin/contrib permission model, element blocks (title, text, alert, Mermaid diagram, application/iframe embeds, button), publishing and moving a page between portals, and live browser verification. Portals v1 is out of scope. |
