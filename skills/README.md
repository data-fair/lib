...
| [`data-fair-browse`](./data-fair-browse/SKILL.md) | Driving a running data-fair platform through a browser under a non-human identity (`@data-fair/nhi-proxy`): what the NHI session implies, the ladder from in-page WebMCP tools (`navigator.modelContext`) to direct API calls to raw Playwright, which tools each surface exposes (portal, data-fair back-office, portals-manager), VJSF form tools and `subagent_*` tools, and how to verify a real rendering. |
| [`portals-pages`](./portals-pages/SKILL.md) | Creating and editing content pages on a data-fair **portals v2** portal from the manager (`/portals-manager`): page-config API, admin/contrib permission model, element blocks (title, text, alert, Mermaid diagram, application/iframe embeds, button), composing harmonious pages (hero, full-width banners, section rhythm, card grids, theme-safe colors across dark/high-contrast themes), editing standard pages (home, contact…), publishing and moving a page between portals, transferring it between departments (sandbox → target), image upload and sizing, annotated screenshots for documentation pages (`scripts/annotate.js` with a geometry check), table of contents and breadcrumbs, sorted listings, and live browser verification. Portals v1 is out of scope. |
