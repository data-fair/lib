...
| [`portals-pages`](./portals-pages/SKILL.md) | Creating and editing content pages on a data-fair **portals v2** portal from the manager (`/portals-manager`): page-config API, admin/contrib permission model, element blocks (title, text, alert, Mermaid diagram, application/iframe embeds, button), editing standard pages (home, contact…), publishing and moving a page between portals, transferring it between departments (sandbox → target), image upload and sizing, annotated screenshots for documentation pages (`scripts/annotate.js` with a geometry check), table of contents and breadcrumbs, sorted listings, and live browser verification. Portals v1 is out of scope. |
