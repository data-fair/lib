...
/** @type {Record<string, {resolve: (data: any) => void, i: number}[]>} */
...
export function waitFor (eventName, timeout = 2000) {
  ...
  const p = new Promise((resolve, reject) => {
    ...
    pendingPromises[eventName].push({ resolve, i })
    setTimeout(() => reject(new Error(`Timeout waiting for test spy ${eventName}`)), timeout)
  })
  p.finally(() => {
    pendingPromises[eventName] = pendingPromises[eventName].filter(p => p.i !== i)
